...
const Request = (request) => {
	...
	const handleAccept = async (e) => {
		e.preventDefault();
		try {
			await axios.post("http://localhost:8800/api/requests/" + request.request._id, { status: "Accepted" }, { withCredentials: true });
			await axios.delete("http://localhost:8800/api/requests/" + request.request._id, { withCredentials: true });
		} catch (err) {
				...
				{request.request.receiverID === user._id ? (
					...
					<div className="body">
						<div className="subbody">
							...
							<div className="status">{request.request.status}</div>
						</div>
					</div>
				)}
			</div>
